...
const makeRenderPublicDir = () => {
  ...
  for (const relative of ["brainrot.mp4", "demo.mp4", "demo-last.png", "narration.mp3", "captions.json", "fonts/Arial-Bold.ttf"]) {
...
const main = () => {
  for (const file of ["public/brainrot.mp4", "public/demo.mp4", "public/demo-last.png", "public/narration.mp3", "public/captions.json"]) {
  ...
  writeFileSync(inputProps, JSON.stringify({captions: JSON.parse(readFileSync("public/captions.json", "utf8"))}));
