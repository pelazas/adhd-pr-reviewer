...
- Open on the GitHub pull request (`startUrl`). Private GitHub PRs are rendered from `gh pr view` as a local card. Later beats `goto` the preview or live site. Hosts are allowed only if they already appear as `startUrl` or a `goto` URL in this plan (plus localhost).
