...
import {Composition, Img, Sequence, staticFile, type CalculateMetadataFunction} from "remotion";
...
type Props = {captions: Caption[]; demoDurationInFrames?: number};
...
const metadata: CalculateMetadataFunction<Props> = async ({props}) => {
  ...
  return {
    ...
    props: {...props, demoDurationInFrames: Math.ceil(demo.durationInSeconds * 30)},
  };
};

export const ADHDReview: React.FC<Props> = ({captions, demoDurationInFrames = 1}) => (
  <div style={{backgroundColor: "#17130f", height: "100%", width: "100%", overflow: "hidden"}}>
    <div style={topStyle}>
      ...
      <Sequence durationInFrames={demoDurationInFrames} layout="none">
        <Video src={staticFile("demo.mp4")} muted objectFit="cover" style={mediaStyle} />
      </Sequence>
    </div>
    <div style={bottomStyle}>
      <Video src={staticFile("brainrot.mp4")} muted loop objectFit="cover" style={mediaStyle} />
    </div>
...
const topStyle: React.CSSProperties = {position: "absolute", top: 0, left: 0, width: 1080, height: 1056, overflow: "hidden"};
